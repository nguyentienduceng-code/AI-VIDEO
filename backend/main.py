...
class RenderVideoRequest(BaseModel):
    ...
    #   mixed       = xen kẽ thông minh video thật và ảnh AI (mặc định đề xuất)
    ...
    # Literary Mode: chọn SFX theo thể loại sách (book|finance|history|psychology|
    # truecrime|travel|...). Pydantic mặc định BỎ IM LẶNG field lạ nên frontend gửi
    # `contentNiche` mà thiếu khai báo ở model thì build_render_kwargs() văng
    # AttributeError NGOÀI khối try → job render treo ở "pending" vĩnh viễn và FE không
    # hiện lỗi. Xem comment tương tự ở intro_bgm_track ngay trên.
...
def build_render_kwargs(req, aspect_ratio: str, mode: str, master_audio_path: str | None) -> dict:
    ...
    kwargs.update(
        ...
        content_niche=req.content_niche,  # Literary Mode: chọn SFX theo thể loại sách
    )
    return kwargs
...
def _pick_visual_source(req, scene: dict, scene_index: int) -> str:
    ...
    if source == "mixed":
        ...
        # Nếu không có thông tin emotion: xen kẽ thông minh (cảnh lẻ dùng video stock, cảnh chẵn dùng ảnh AI)
        return "stock_video" if scene_index % 2 == 1 else "ai_image"
...
def _compose_speech_rate(user_rate: str, scene_modifier: str) -> str:
    ...
    def _parse(v: str) -> int:
        ...
        n = int(m.group(1))
        if not (-50 <= n <= 50):
            return max(-50, min(50, n))  # clamp out-of-range silently
        return n

    total = _parse(user_rate) + _parse(scene_modifier)
    total = max(-50, min(50, total))  # double-clamp for safety
    return f"{total:+d}%"
